...
function statussuccess(response) {
    id('refreshstatusbtn').style.display = 'block';
    id('status_loader').style.display = 'none';
    var modal = getactiveModal();
    if (modal == null) return;
    var text = modal.element.getElementsByClassName("modal-text")[0];
...
function statusfailed(errorcode, response) {
    id('refreshstatusbtn').style.display = 'block';
    id('status_loader').style.display = 'none';
    id('status_msg').style.display = 'block';
    console.log("Error " + errorcode + " : " + response);
    id('status_msg').innerHTML = "Error " + errorcode + " : " + response;
}

function refreshstatus() {
    id('refreshstatusbtn').style.display = 'none';
    id('status_loader').style.display = 'block';
    var modal = getactiveModal();
    if (modal == null) return;
    var text = modal.element.getElementsByClassName("modal-text")[0];
    text.innerHTML = "";
    id('status_msg').style.display = 'none';
    //removeIf(production)
